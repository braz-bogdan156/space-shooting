...
export const moveRandomBossBullets = () => {
  for (let i = gameState.randomBossBullets.length - 1; i >= 0; i--) {
    ...
    if (
      gameState.randomBossBullets[i].x < 0 ||
      gameState.randomBossBullets[i].x > app.screen.width ||
      gameState.randomBossBullets[i].y < 0 ||
      gameState.randomBossBullets[i].y > app.screen.height
    ) {
